...
def test_3b(answer1,answer2):
    assert isinstance(answer1, int),  str(answer) + \
        " should be expressed as an integer"
    assert isinstance(answer2, int),  str(answer) + \
        " should be expressed as an integer"
...
def test_7g(answer):
    assert isinstance(answer, object), "This is not the correct type onf output"
    assert answer.get_title() == 'Distribution of Male Runners BMI', "You have not put in the correct title"
    return "Success"

def test_7h(answer):
    assert isinstance(answer, object), "This is not the correct type onf output"
    assert answer.get_title() == 'Relationship between BMI and 10k running time (mins)'
    return "Success"
