...
def test_3a():
    with open("sampling_tests.py") as fp:
        for i, line in enumerate(fp):
            if i == 1:
                break
    assert "sampling" in str(line), "Make sure you are importing from the 'sampling.py' file"
    assert "sample_dataframe" in str(line), "Make sure you are the 'sample_dataframe' function from the 'sampling.py' file"
...
def test_4aii(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(str(answer + 9).encode('utf8')).hexdigest() == "7b52009b64fd0a2a49e6d8a939753077792b0554", "Your answer is incorrect. Please try again."
    return("Success")
